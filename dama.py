...
def Main():
    ...
    parser.add_argument('usage', help='Use "setcmd" to adjust the size of the window and position it above the rest of the applications, "dolphin.start" to initialize Dolphin Anty, "dolphin.kill" to kill all Dolphin Anty processes, "profiles.start" to initialize all profiles, "profiles.close" to close all profiles, "profiles.close.dolphin.kill" to close all profiles and kill all Dolphin processes, "twitter.followbot" to follow followers of the configured prompts, "twitter.mg-farmer" to like posts that match the configured prompts, "twitter.unfollowbot" to unfollow users who do not follow you, "twitter.unfollowbot.all" to unfollow without exceptions.' )
